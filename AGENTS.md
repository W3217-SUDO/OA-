...
## 本地预览与发布授权规则（优先于本文件的自动发布条款）

- 用户要求“先本地优化”“本地预览”“不满意可回滚”或明确表示“未要求提交线上”时，本轮工作只能修改本地工作树并完成本地测试、构建和浏览器预览；必须保持改动范围清晰、可单独回滚。
- 在上述本地预览阶段，未经用户后续明确授权，禁止执行 `git add`、`git commit`、`git push`、版本号递增、服务器文件同步、服务重启或向 `8089`/其他线上入口部署。
- 本规则是对下文“验证后自动提交/部署”等条款的明确例外；发生项目级规则冲突时，以本规则和用户当前最新指令为准。系统级、开发者级安全与权限指令仍具有更高优先级。
- 用户确认本地预览满意并明确要求提交或部署后，才恢复执行版本号、精确暂存、提交、部署、健康检查和线上浏览器验收流程。

## 旧系统数据迁移关联门禁

- 旧系统数据迁移必须按旧表主键、GUID、业务编号和页面查询源码恢复完整关系链，至少覆盖客户 -> 合同 -> 案件 -> 调查 -> 调查任务 -> 调查线索 -> 取证；禁止根据文件名或物理路径猜测业务归属。
- 文件必须同时恢复旧表投影和新系统 `FileAttachment.record_id` 关联。客户文件挂客户记录，合同文件挂合同记录，案件文件挂案件记录，调查/线索/取证文件挂各自业务记录；案件详情的关联目录还必须按页面实际读取的父记录验收。
- 导入器必须幂等，并在同一事务内校验所有父记录、软键、`ContractObject`、业务记录 data 关联和附件归属。任一父级缺失、文件哈希不一致、目录查询无结果或附件归属错误时，整批失败并回滚，不得留下“记录已导入但未关联”的半成品。
- 每次迁移完成前必须执行数据库关联审计和 Codex 内置浏览器验收，逐项打开客户文档、合同文档、案件文档、调查文档、鉴别资料和取证文档；数据库有行或磁盘有文件不能单独作为完成证据。
...
- `http://150.158.3.104:8091/` is the online legacy-system reference only. It may be inspected to establish business behavior and layout, but it must never be modified, restarted, redeployed, or receive local legacy-source changes. The sole production implementation target remains the new OA system on the `dev` worktree and its `8089` deployment.
- The locally deployed legacy-system copy at `http://localhost:8091/` or `http://127.0.0.1:8091/`, together with its local source directory, may be modified only when the user explicitly requests that local legacy copy to be changed. Such changes are local reference/testing changes only and must never be synchronized or deployed to the online legacy system at `150.158.3.104:8091`.
...
- **False-completion recovery:** A user report that the same issue is still wrong, not changed, or inconsistent with the legacy system, including a new screenshot of that problem, explicitly reopens the row even if its status is `1`. The row must no longer be skipped and cannot return to `1` until it passes the full acceptance gate again.
- **Status-1 hard gate:** Never derive `1` from a code change, successful API response, persisted value, self-authored test note, build, or deployment alone. Re-open the original row text and every anchored screenshot immediately before writing status. Map every requirement to the exact changed code and direct browser evidence; any unmapped requirement leaves the row unfinished. Write `1` for one independently accepted row at a time, never as a bulk batch update.
- **Legacy parity gate:** When a row says to follow, reference, or match the legacy system, trace the legacy page/template through its controller, service, repository, database schema, permission and workflow logic, then compare the complete new-system chain. Field names, order, control types, required rules, defaults, buttons and layout, plus request payloads, backend validation, permissions, data scope, state transitions, database fields/relationships, approvals, logs, notifications, attachments, downstream visibility and persisted results must be equivalent unless the user explicitly requests a difference. Screenshots are requirement and UI evidence, not the whole implementation standard. The acceptance screenshot must visibly contain the target window and its fields, while API/database checks and real role-based workflow acceptance must separately prove the business chain; none can substitute for the others.
