...
- 实际改动提交：`884ea75542262c3bc88b7c18f1f31a50928ec658`。
- 预留发布版本：`v1.1.120`。
- 发布状态：待正式激活及用户验收。

## 10. 用户补充返工（旧系统新增提成布局与对应关系）

- 旧系统权威入口：`Legal/InternalFee/MultiCreateByCaseFee`，右侧工作区固定约 700px；表格列为案号、费用类型、支付对象、基数、参考提成、实际金额、备注、操作。
- 旧系统表头支持把第一行的费用类型、基数、实际金额、备注同步到各行；基数变化会重新计算参考提成和实际金额。
- 高开金额来自当前案件费用关联的有效开票分配，高开成本按旧系统 `InvoiceOverAmountRate=14.28%` 计算，提成基数为所选代理费减高开成本。
- 法院退款按案件全部外部费用的有效退费申请减已退款金额汇总，不能只读取所选代理费快照。
- 品管提成的支付对象必须读取案件关联客户基本信息中最新的品牌管理人；优先读取最后一次管理人分配记录，再读取当前 `customer_managers` 第一位/客户 owner，不能使用案件内过期的协调人快照。
- 已只读核对用户截图中的案件：案件保存了明确的客户 ID/客户编号；客户最后一次管理人分配的 `to_owner` 与当前 `customer_managers[0]`、客户 owner 一致，并存在案件日期有效的 `quality_rate` 提成方案，具备生成品管提成的完整权威链路。
- 新系统已补充右侧 700px 紧凑布局、可交互费用类型、支付对象标签、黄色基数/实际金额、表头同步和基数重算；后端提交时重新验证提成模板并按提交基数计算参考提成。
- 本轮改动提交：`047eb06c8e05851826756de34126f90c002bd82e`。
- `v1.1.124` 激活时服务健康检查已通过，但日志检查误把 systemd 的“无日志条目”提示识别为错误，已按发布规则自动回滚并占用该版本。
- 本轮修正发布版本：`v1.1.125`。
- 本轮发布状态：待正式激活及用户验收。
