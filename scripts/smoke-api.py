...
def main():
    ...
    try:
        ...
        atomic_employee = call("POST", "/hr/employees", {"username": atomic_employee_name, "display_name": "原子新建员工", "employee_no": serial("HR-ATOMIC"), "company": "上海申浩律师事务所", "department": active_department["name"], "password": "SmokePass2026!", "role": "user", "position": active_job_role["name"], "is_active": True, "data": {"joined_at": str(date.today()), "mobile": "13800000001"}}, expected=(201,))
        users.append(atomic_employee["user"]["id"]); records.append(atomic_employee["employee"]["id"])
        assert atomic_employee["employee"]["owner"] == atomic_employee_name and atomic_employee["user"]["username"] == atomic_employee_name
        call("POST", "/hr/employees", {"username": atomic_employee_name, "display_name": "重复员工", "employee_no": serial("HR-ATOMIC-2"), "company": "上海申浩律师事务所", "department": active_department["name"], "password": "SmokePass2026!", "role": "user", "position": active_job_role["name"], "is_active": True, "data": {}}, expected=(409,))
