...
test('employee detail tabs refresh their active subrecord list like the legacy staff tabs', () => {
  ...
  assert.match(source, /<Tabs activeKey=\{detailTab\} onChange=\{setDetailTab\} destroyOnHidden items=/)
  assert.match(source, /kind="leave" canManage=\{actionAccess\.canProcessStatus\}\/>/)
  assert.match(source, /kind="archive" canManage=\{actionAccess\.canProcessStatus\}\/>/)
  assert.match(source, /kind="matter" canManage=\{actionAccess\.canProcessStatus\}\/>/)
  assert.match(source, /kind="commission" canManage=\{actionAccess\.canProcessStatus\}\/>/)
})
