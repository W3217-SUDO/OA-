...
test("new contract wizard keeps the legacy four-step approval and seal flow", () => {
  ...
  assert.match(contractPage, /保存同步用印资料/);
  assert.match(contractPage, /const \{ submit: submitFromForm, \.\.\.sealValues \} = values/);
  assert.match(contractPage, /const deferSyncSealSubmission = wizardDraft\.status === "审批中" && Boolean\(wizardDraft\.data\.sync_seal\);/);
  assert.match(contractPage, /const submitApplication = deferSyncSealSubmission \? false : forcedSubmit \?\? Boolean\(submitFromForm\);/);
  ...
  assert.match(contractPage, /合同审批通过后将自动提交用印审批/);
  ...
  assert.match(contractPage, /请保存用印资料为草稿；合同审批通过后，系统会自动提交用印审批/);
