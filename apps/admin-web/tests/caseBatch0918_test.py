...
class Batch0918Test(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_merge_all_relations_and_keep_main_values(self):
        ...
        self.assertEqual(result.status_code, 200, result.text)
        self.assertEqual(result.json()["target"]["id"], self.case_id)
        repeated = await self.client.post(f"{API}/cases/{self.case_id}/merge", json={"source_case_no": "CODEX-0918-source"})
