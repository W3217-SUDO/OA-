...
export default function SystemCenterPage({
  ...
  const clearSelectedCaches = async () => {
    ...
    setCacheActionPending(true);
    try {
      // The compatibility batch endpoint validates the complete selection first,
      // so a direct-query row cannot leave a partly-cleared selection behind.
      ...
      <Card className="panel system-focused" title="缓存管理">
        ...
        <Table
          ...
          rowSelection={{
            ...
            onChange: (keys) => setSelectedCacheKeys(keys.map(String)),
            getCheckboxProps: (row: CacheRow) => ({ disabled: !row.clearable }),
          }}
          title={() => (
            <Space>
              <Button disabled={cacheActionPending} onClick={() => void loadCaches(cachePage, cachePageSize)} icon={<ReloadOutlined />}>刷新列表</Button>
              <Popconfirm title="确认清除选中的缓存？" onConfirm={clearSelectedCaches}>
          ...
          columns={[
            ...
            { title: "上次清理", dataIndex: "last_cleared_at", width: 175, render: (value) => formatTime(value) },
            {
              title: "操作",
              key: "action",
              width: 110,
              render: (_v, row: CacheRow) => (
                row.clearable ? (
                  <Popconfirm title="确认清空此缓存？" onConfirm={() => clearCache(row)}>
