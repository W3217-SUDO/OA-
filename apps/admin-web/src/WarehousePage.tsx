...
type StorageLocation={id:number;storage_location_no:string;name:string;address:string;is_active:boolean;goods_count:number}
type WarehouseCatalogItem={id:number;warehouse_no:string;name:string;address:string;is_active:boolean;goods_count:number;locations:StorageLocation[]}
...
export default function WarehousePage({onNavigate}:{onNavigate?: (route:string)=>void}){
  ...
  const [catalog,setCatalog]=useState<WarehouseCatalogItem[]>([])
  const [loading,setLoading]=useState(false)
  const [warehouseId,setWarehouseId]=useState<number|undefined>()
  const [storageLocationId,setStorageLocationId]=useState<number|undefined>()
  ...
  const [formWarehouseId,setFormWarehouseId]=useState<number|undefined>()
  const [actionWarehouseId,setActionWarehouseId]=useState<number|undefined>()
  const [form]=Form.useForm()
  const [actionForm]=Form.useForm()

  const load=async(nextPage=page,nextPageSize=pageSize,selection?:{warehouseId?:number;storageLocationId?:number})=>{setLoading(true);try{
    const selected=selection||{warehouseId,storageLocationId}
    const params={page:nextPage,page_size:nextPageSize,warehouse_id:selected.warehouseId,storage_location_id:selected.storageLocationId,rights_holder:rightsHolder,evidence_status:status,case_no:caseNo,shop_name:shop,investigator,notary_no:notaryNo,evidence_date_from:dates?.[0]?.format('YYYY-MM-DD'),evidence_date_to:dates?.[1]?.format('YYYY-MM-DD')}
    const [catalogResponse,evidenceResponse]=await Promise.all([api.get('/warehouse/catalog'),api.get('/warehouse/evidence',{params})])
    const catalogItems=Array.isArray(catalogResponse.data.items)?catalogResponse.data.items as WarehouseCatalogItem[]:[]
    setCatalog(catalogItems)
    setAllRows(Array.isArray(evidenceResponse.data.items)?evidenceResponse.data.items as Item[]:[])
    setTotal(Number(evidenceResponse.data.total)||0)
  }catch{message.error('仓库数据加载失败')}finally{setLoading(false)}}
  useEffect(()=>{void load()},[])
  const storageTreeData=useMemo<TreeDataNode[]>(()=>{
    return catalog.map(warehouse=>({key:`warehouse:${warehouse.id}`,title:`${warehouse.name}【${warehouse.goods_count}】`,children:warehouse.locations.map(location=>({key:`location:${warehouse.id}:${location.id}`,title:`${location.name}【${location.goods_count}】`}))}))
  },[catalog])
  ...
  const openEditor=(row?:Item)=>{
    ...
    form.resetFields()
    const d=row?.data||{}
    const selectedWarehouseId=Number(d.warehouse_id)||undefined
    setFormWarehouseId(selectedWarehouseId)
    if(row)form.setFieldsValue({serial_no:row.serial_no,warehouse_id:selectedWarehouseId,storage_location_id:Number(d.storage_location_id)||undefined,notary_no:d.notary_no||'',case_no:d.case_no||'',shop_name:d.shop_name||row.title,investigator:d.investigator||row.owner,notary_office:d.notary_office||'',rights_holder:d.rights_holder||'',evidence_date:d.evidence_date?dayjs(String(d.evidence_date)):null,description:row.description||''})
    setEditorOpen(true)
  }
  ...
  const openAction=(kind:ActionKind,row:Item)=>{setAction({kind,row});actionForm.resetFields();const d=row.data||{};const selectedWarehouseId=Number(d.warehouse_id)||undefined;setActionWarehouseId(selectedWarehouseId);if(kind==='check-in'||kind==='recheck-in')actionForm.setFieldsValue({warehouse_id:selectedWarehouseId,storage_location_id:Number(d.storage_location_id)||undefined,condition:'完好'});setTimeout(()=>actionForm.getFieldInstance(kind==='destroy'?'reason':kind==='check-out'?'recipient':'warehouse_id')?.focus?.(),0)}
  ...
  const columns:TableColumnsType<Item>=[
    ...
    {title:'线索编号',key:'clueNo',width:150,render:(_v,row)=>{const clueNo=String(row.data.clue_no||'').trim();return clueNo?<Button type="link" onClick={()=>openClueDetail(clueNo)}>{clueNo}</Button>:'—'}},
  ...
  const selectStorageLocation=(keys:Key[])=>{
    ...
    if(key.startsWith('warehouse:')){const nextWarehouseId=Number(key.slice('warehouse:'.length));setWarehouseId(nextWarehouseId);setStorageLocationId(undefined);setPage(1);void load(1,pageSize,{warehouseId:nextWarehouseId});return}
    if(key.startsWith('location:')){const [,warehouseValue,locationValue]=key.split(':');const nextWarehouseId=Number(warehouseValue);const nextStorageLocationId=Number(locationValue);setWarehouseId(nextWarehouseId);setStorageLocationId(nextStorageLocationId);setPage(1);void load(1,pageSize,{warehouseId:nextWarehouseId,storageLocationId:nextStorageLocationId})}
  }

  return <Card className="panel warehouse-location" title="仓库库位">
    <div className="warehouse-layout">
      <aside className="warehouse-storage-tree" aria-label="仓库库位汇总">
        <div className="warehouse-storage-title"><span>仓库库位</span><Button type="link" size="small" onClick={()=>{setWarehouseId(undefined);setStorageLocationId(undefined);setPage(1);void load(1,pageSize)}}>全部</Button></div>
        <Tree blockNode virtual={false} selectedKeys={[storageLocationId?`location:${warehouseId}:${storageLocationId}`:warehouseId?`warehouse:${warehouseId}`:'']} onSelect={selectStorageLocation} treeData={storageTreeData}/>
      ...
      <div className="warehouse-query">
        <label><span>仓库</span><Select value={warehouseId} onChange={value=>{setWarehouseId(value);setStorageLocationId(undefined)}} allowClear options={catalog.map(value=>({value:value.id,label:value.name,disabled:!value.is_active}))}/></label>
        <label><span>库位</span><Select value={storageLocationId} onChange={setStorageLocationId} allowClear options={(catalog.find(item=>item.id===warehouseId)?.locations||[]).map(value=>({value:value.id,label:value.name,disabled:!value.is_active}))}/></label>
        ...
        <Space>
          <Button type="primary" onClick={()=>{setPage(1);void load(1,pageSize)}}>查询</Button>
          <Button onClick={()=>openEditor()}>登记证物</Button>
        </Space>
      </div>
      <Table rowKey="id" size="small" loading={loading} columns={columns} dataSource={allRows} locale={{emptyText:<Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="没有查询到符合条件的记录 。"/>}} pagination={{current:page,pageSize,total,showSizeChanger:true,pageSizeOptions:['10','15','20','50','100','200'],showQuickJumper:true,showTotal:count=>`共 ${count} 条`,onChange:(nextPage,nextPageSize)=>{setPage(nextPage);setPageSize(nextPageSize);void load(nextPage,nextPageSize)}}} scroll={{x:1500}}/>
      </section>
    </div>
    ...
    <Modal width={760} title={editing?'编辑证物资料':'登记证物'} open={editorOpen} confirmLoading={saving} onCancel={()=>setEditorOpen(false)} onOk={()=>void saveEvidence()} okText="保存" cancelText="取消" destroyOnHidden>
      <Form form={form} layout="vertical" className="warehouse-evidence-form">
        ...
        <Form.Item label="仓库" name="warehouse_id" rules={[{required:true,message:'请选择仓库'}]}><Select options={catalog.filter(item=>item.is_active).map(value=>({value:value.id,label:value.name}))} onChange={value=>{setFormWarehouseId(value);form.setFieldValue('storage_location_id',undefined)}}/></Form.Item>
        <Form.Item label="库位" name="storage_location_id" rules={[{required:true,message:'请选择库位'}]}><Select options={(catalog.find(item=>item.id===formWarehouseId)?.locations||[]).filter(item=>item.is_active).map(value=>({value:value.id,label:value.name}))}/></Form.Item>
    ...
    <Modal title={actionTitle} open={Boolean(action)} confirmLoading={saving} onCancel={()=>setAction(null)} onOk={()=>void submitAction()} okText="确认办理" okButtonProps={{danger:action?.kind==='destroy'}} destroyOnHidden>
      {action&&<><Descriptions size="small" column={1} bordered items={[{key:'serial',label:'线索编号',children:action.row.data.clue_no?<Button type="link" onClick={()=>openClueDetail(action.row.data.clue_no)}>{String(action.row.data.clue_no)}</Button>:'—'},{key:'shop',label:'货物出售店铺',children:String(action.row.data.shop_name||action.row.title)},{key:'status',label:'当前状态',children:<Tag color={statusColor[statusOf(action.row)]}>{statusOf(action.row)}</Tag>}]}/><Form form={actionForm} layout="vertical" className="warehouse-action-form">
        {(action.kind==='check-in'||action.kind==='recheck-in')&&<><Form.Item label="仓库" name="warehouse_id" rules={[{required:true,message:'请选择仓库'}]}><Select options={catalog.filter(item=>item.is_active).map(value=>({value:value.id,label:value.name}))} onChange={value=>{setActionWarehouseId(value);actionForm.setFieldValue('storage_location_id',undefined)}}/></Form.Item><Form.Item label="库位" name="storage_location_id" rules={[{required:true,message:'请选择库位'}]}><Select options={(catalog.find(item=>item.id===actionWarehouseId)?.locations||[]).filter(item=>item.is_active).map(value=>({value:value.id,label:value.name}))}/></Form.Item></>}
