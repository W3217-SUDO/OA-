...
export default function BusinessPage({module,title,openCreate=false,defaultStatus='',onNavigate}:{module:string;title?:string;openCreate?:boolean;defaultStatus?:string;onNavigate?: (route:string)=>void}){
  ...
  const columns=useMemo(()=>[
    ...
    {title:'客户/主体',dataIndex:'customer',width:190,ellipsis:true},
    ...
    ...config.fields.slice(0,2).map(field=>({title:field.label,key:field.key,width:130,ellipsis:true,render:(_:unknown,row:RecordRow)=>String(row.data?.[field.key]??'')})),
