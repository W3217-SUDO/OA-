...
export function CustomerCreateEditModal({
  ...
  const renderCustomerRelatedTabs = (showSaveButton: boolean) => (
    <Tabs
      ...
      tabBarExtraContent={showSaveButton ? <Button type="primary" onClick={onSave}><span>保</span><span>存</span></Button> : undefined}
      items={[
        {
          key: "contacts",
          label: "联系人",
          children: (
            <>
              ...
              <Table
                ...
                scroll={{ x: 1460 }}
                locale={{ emptyText: <span>没有查询到联系人，可以去 <Button type="link" onClick={() => onNewEditor("contact")}>新建联系人</Button></span> }}
                columns={[
                  ...
                  { title: "照片", width: 150, render: (_: unknown, row: Contact) => contactPhotoActions(row) },
                  {
                    ...
                    render: (_: unknown, row: Contact) => canManage ? (
                      <Space size={0}>
                        ...
                        <Popconfirm title="删除联系人？" onConfirm={() => onDeleteContact(row.id)}>
          ...
          children: (
            <>
              <Table
                ...
                scroll={{ x: 720 }}
                locale={{ emptyText: <span>没有查询到事项记录，可以去 <Button type="link" onClick={() => onNewEditor("note")}>新建</Button></span> }}
                columns={[
                  ...
                  {
                    title: "操作",
                    render: (_: unknown, row: Note) => (
                      <Popconfirm title="删除这条记录？" onConfirm={() => onDeleteNote(row.id)}>
                        <Button type="link" danger>删除</Button>
                      </Popconfirm>
                    ),
                  },
          ...
          children: (
            <>
              ...
              <Table
                ...
                scroll={{ x: 720 }}
                locale={{ emptyText: <span>没有查询到客户文件，可以去 <Button type="link" onClick={() => onNewEditor("document")}>上传客户文件</Button></span> }}
                columns={[
                  ...
                  {
                    title: "操作",
                    render: (_: unknown, row: Attachment) => canDeleteCustomerAttachment(canManage) ? (
      ...
      <Form form={form} layout="horizontal" className="customer-create-form">
        <section>
          ...
          <div className="customer-create-grid">
            ...
            <Form.Item label="客户编码" name="serial_no" rules={isEdit ? [{ required: true }] : []}>
              <Input disabled={isEdit} placeholder="自动生成" />
            </Form.Item>
            <Form.Item label="客户状态" name="status">
              <Select allowClear placeholder="请选择" options={["潜在", "目标", "立项", "关怀", "签约", "谈判", "价值"].map(value => ({ value, label: value }))} />
            </Form.Item>
        ...
        <section>
          ...
          <div className="customer-create-grid">
            ...
            <Form.Item label="统一社会信用代码" name="credit_code">
              <Input disabled={isEdit} placeholder="不允许有空格." />
            </Form.Item>
        ...
        <section>
          ...
          <div className="customer-create-grid customer-control-grid">
            ...
            <Form.Item label="客户来源" name="customer_source">
              <Select showSearch optionFilterProp="label" options={directoryOptions} filterOption={matchesDirectoryOption} placeholder="输入或选择人员" />
            </Form.Item>
            <Form.Item label="是否共享" name="is_shared">
              <Select options={["是", "否"].map(value => ({ value, label: value }))} />
            </Form.Item>
            <Form.Item label="客户等级" name="level">
              <Select options={["签约客户", "立案客户", "高级客户", "中级客户", "低级客户"].map(value => ({ value, label: value }))} />
            </Form.Item>
            <Form.Item label="上海市资助信息" name="is_assisted">
              <Select options={["是", "否"].map(value => ({ value, label: value }))} />
            </Form.Item>
            <Form.Item
            ...
            <Form.Item className="customer-person-multi-field" label="客户联系人账号" name="contact">
              <Select mode="multiple" showSearch optionFilterProp="label" options={customerContactOptions} filterOption={matchesDirectoryOption} placeholder="输入姓名或账号，选择客户账号" />
            </Form.Item>
          </div>
        </section>
      </Form>
