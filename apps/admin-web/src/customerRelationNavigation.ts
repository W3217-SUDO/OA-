...
const STRICT_MODE_REPLAY_MS = 2 * 1000;
let replayContext: CustomerRelationNavigationContext | null = null;
let replayed = false;
...
export const consumeCustomerRelationTarget = (
  expectedTarget?: CustomerRelationNavigationContext["target"],
): CustomerRelationNavigationContext | null => {
  const raw = sessionStorage.getItem(STORAGE_KEY);
  if (!raw) {
    if (
      replayContext
      && !replayed
      && (!expectedTarget || replayContext.target === expectedTarget)
      && Date.now() - replayContext.at <= STRICT_MODE_REPLAY_MS
    ) {
      replayed = true;
