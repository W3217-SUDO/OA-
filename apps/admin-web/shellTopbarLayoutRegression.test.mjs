...
assert.match(css, /\.topbar-workspace-tabs \{[\s\S]*?height: 50px;[\s\S]*?flex: 1 1 480px;/);
assert.match(css, /\.topbar-workspace-tabs\.ant-tabs-card > \.ant-tabs-nav \.ant-tabs-tab \{[\s\S]*?height: 50px;/);
assert.match(css, /@media \(max-width: 1200px\)[\s\S]*?\.case-quick-search \{[\s\S]*?display: none !important;/);
