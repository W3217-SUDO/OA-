...
test('receivables contract numbers open the receivables-detail route with an exact contract filter', () => {
  ...
  assert.match(source, /buildReceivableDetailContext\(contractNo, initialView(?:, amountFilter)?\)/)
  assert.match(source, /sessionStorage\.setItem\("sunhold:receivable-detail-context", JSON\.stringify\(context\)\)/)
  assert.match(source, /setDetailContext\(context\)/)
