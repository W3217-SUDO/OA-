...
class ContractLifecycleTest(unittest.TestCase):
    def test_draft_approval_change_archive_and_attachment(self):
        ...
        try:
            ...
            status, _, body = request("POST", f"/contracts/{contract_ids[0]}/changes", token, {"change_type": "amend", "reason": "scope", "amount": 100})
            self.assertEqual(status, 201); self.assertEqual(json.loads(body)["status"], "已生效"); self.assertTrue(json.loads(body)["applied"])
            changed = json.loads(request("GET", f"/records/{contract_ids[0]}", token)[2]); self.assertEqual(changed["data"]["amount"], 100)
            ...
            changed = json.loads(request("GET", f"/records/{contract_ids[0]}", token)[2]); self.assertEqual(changed["title"], f"{prefix}-合同-1-变更")
