...
class OrdinaryUserSealMenuContractTest(unittest.TestCase):
    ...
    def test_configured_assistant_permissions_are_restored(self) -> None:
        self.assertIn("assistant_configured_seal_permissions_restore_v1", SOURCE)
        self.assertIn('"seal", "seal-my", "seal-audit", "seal-admin"', SOURCE)
        self.assertIn('"seal-admin-pending", "seal-admin-used", "seal-admin-query", "用印审批"', SOURCE)
